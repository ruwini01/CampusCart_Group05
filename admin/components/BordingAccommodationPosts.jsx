...
const BordingAccommodationPosts = ({ searchQuery }) => {
  ...
  // Load seen posts from AsyncStorage when component mounts
  useEffect(() => {
    const loadSeenPosts = async () => {
      try {
        const storedSeenPosts = await AsyncStorage.getItem(STORAGE_KEY);
        ...
        renderItem={({ item }) => (
          <TouchableOpacity activeOpacity={0.7} className="p-4" onPress={() => handlePress(item)}>
            <View className="relative">
              ...
              />
              {/* Render the green circle if post hasn't been seen */}
