...
const STORAGE_KEY_FOUND = 'seenFoundPosts';
...
const FoundPosts = ({ searchQuery }) => {
  ...
  useEffect(() => {
    const loadSeenPosts = async () => {
      try {
        const storedSeenPosts = await AsyncStorage.getItem(STORAGE_KEY_FOUND);
  ...
  const saveSeenPosts = async (postsArray) => {
    try {
      await AsyncStorage.setItem(STORAGE_KEY_FOUND, JSON.stringify(postsArray));
    } catch (error) {
            ...
            <View className="relative">
              <Card
                ...
                name={
                  ...
                  item.location +
                  ' on ' +
                  item.founddate.slice(0, 10)
                }
                days={calculateTimeAgo(item.date)}
              />
              {/* Render a green circle if the post hasn't been seen */}
